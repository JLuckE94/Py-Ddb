...
def remove_actors(title, year, actor_count, dynamodb=None):
    ...
    try:
        response = table.update_item(
            ...
            UpdateExpression="remove info.actors[0]",       #ConditionExpression="size(info.actors) > :num"
            ConditionExpression="size(info.actors) >= :num", #ConditionExpression="size(info.actors) >= :num", 
